...
* Functions can reduce computation by returning lazy ALTREP results that only evaluate entries if they are needed (see the [Deferred String vignette](deferred_string.html))
* Adding metadata to vectors that is invisible and un-editable by users (see the [Wrapper Objects vignette](wrapper_objects.html))
* Abstracting away the actual storage mechanism of a vector, which might not even exist in memory (see the [MemMap vignette](memmap.html]))
...
For those interested in the specifics, an `ALTREP` is actually stored using
the same memory layout as a regular R list.
R lists hold 3 pointers, (called the `CAR`, `CDR` and `TAG`)^[[Defn.h#L145-L149](https://github.com/wch/r-source/blob/7d5117ad947beed213d3d4538bfe36441478ebef/src/include/Defn.h#L145-L149)].
ALTREPs re-use this structure to point to the three parts of an ALTREP^[[Rinlinedfuns.h#L386-L389](https://github.com/wch/r-source/blob/7d5117ad947beed213d3d4538bfe36441478ebef/src/include/Rinlinedfuns.h#L386-L389)]:
  ...
  * `data1`: internal data, as a `SEXPREC` (normal R object)
  * `data2`: more internal data (also a `SEXPREC`)
  * `tag`: The ALTREP class, as a raw vector with attributes
